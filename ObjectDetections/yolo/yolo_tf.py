...
class Yolo(object):
    ...
    def _build_net(self):
        ...
        net = self._maxpool_layer(net, 6, 2, 2)  # 2*2 池化核 步长2            -> 23*23*512
        net = self._conv_layer(net, 7, 256, 1, 1)# 23*23*512 -> 1*1*512*256/1 -> 23*23*256
        net = self._conv_layer(net, 8, 512, 3, 1)# 23*23*256 -> 3*3*256*512/1 -> 23*23*512
        net = self._conv_layer(net, 9, 256, 1, 1)# 23*23*512 -> 1*1*512*256/1 -> 23*23*256
        net = self._conv_layer(net, 10, 512, 3, 1)#23*23*256 -> 3*3*256*512/1 -> 23*23*512
        net = self._conv_layer(net, 11, 256, 1, 1)#23*23*512 -> 1*1*512*256/1 -> 23*23*256
        net = self._conv_layer(net, 12, 512, 3, 1)#23*23*256 -> 3*3*256*512/1 -> 23*23*512
        net = self._conv_layer(net, 13, 256, 1, 1)#23*23*512 -> 1*1*512*256/1 -> 23*23*256
        net = self._conv_layer(net, 14, 512, 3, 1)#23*23*256 -> 3*3*256*512/1 -> 23*23*512
        net = self._conv_layer(net, 15, 512, 1, 1)#23*23*512 -> 1*1*512*512/1 -> 23*23*512
        net = self._conv_layer(net, 16, 1024, 3, 1)#23*23*512-> 3*3*512*1024/1 -> 23*23*1024
        net = self._maxpool_layer(net, 16, 2, 2)   #2*2 池化核 步长2            -> 11*11*1024
        net = self._conv_layer(net, 17, 512, 1, 1) #11*11*1024 -> 1*1*1024*512/1 -> 11*11*512
        net = self._conv_layer(net, 18, 1024, 3, 1)#11*11*512  -> 3*3*512*1024/1 -> 11*11*1024
        net = self._conv_layer(net, 19, 512, 1, 1) #11*11*1024 -> 1*1*1024*512/1 -> 11*11*512
        net = self._conv_layer(net, 20, 1024, 3, 1)#11*11*512  -> 3*3*512*1024/1 -> 11*11*1024
        net = self._conv_layer(net, 21, 1024, 3, 1)#11*11*1024 -> 3*3*1024*1024/1-> 11*11*1024
        net = self._conv_layer(net, 22, 1024, 3, 2)#11*11*1024 -> 3*3*1024*1024/2-> 7*7*1024
